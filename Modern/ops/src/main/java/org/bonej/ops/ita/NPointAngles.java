...
public class NPointAngles extends
	AbstractBinaryFunctionOp<List<Vertex>, Integer, List<NPoint>>
{
	...
	/**
	 * This function computes the angles between outgoing edges for each vertex in
	 * a list.
	 *
	 * @param vertices We want to compute angles between connected edges for each
	 *          {@link Vertex} in this list.
	 * @param measurementMode specifies the number of slabs away from the central
	 *          vertex we want to measure the angle to -1 signifies using the
	 *          vertex at the other end of the edge.
	 * @return a list of {@link NPoint} which represent collections of angles
	 *         around a vertex
	 */
	@Override
	public List<NPoint> calculate(final List<Vertex> vertices,
		final Integer measurementMode)
	{
		final List<NPoint> nPoints = new ArrayList<>();
		for (final Vertex v : vertices) {
			final List<VectorsAngle> vectorAngles = computeVectorAngles(v,
				measurementMode);
	...
	private List<VectorsAngle> computeVectorAngles(final Vertex centre,
		final int measurementPoint)
	{
		...
		for (int i = 0; i < edges.size() - 1; i++) {
			for (int j = i + 1; j < edges.size(); j++) {
				final Vector3d centreVector = centroidOp.calculate(GraphUtil.toVector3d(
					centre.getPoints()));
				final Vector3d endVectorI = getMeasurementPoint(centre, edges.get(i),
					measurementPoint);
				final Vector3d endVectorJ = getMeasurementPoint(centre, edges.get(j),
					measurementPoint);

				endVectorI.sub(centreVector);
				endVectorJ.sub(centreVector);

				final VectorsAngle vectorsAngle = new VectorsAngle(endVectorI,
					endVectorJ);
	...
	private Vector3d getOppositeCentroid(final Vertex vertex, final Edge edge) {
		...
		final List<Vector3d> oppositeVertexVectors = GraphUtil.toVector3d(
			oppositeVertex.getPoints());
