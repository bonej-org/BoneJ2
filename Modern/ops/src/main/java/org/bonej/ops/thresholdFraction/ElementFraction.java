...
/**
 * Counts the total number of elements in the interval, elements within
 * thresholds, and their fraction
 *
	...
	AbstractBinaryFunctionOp<IterableInterval<T>, ElementFraction.Settings<S>, ElementFraction.Results>
{
		...
		final Settings<S> settings)
	{
		...
		final Stream<T> elementStream = StreamSupport.stream(interval.spliterator(),
			false);
		final long thresholdElements = elementStream.filter(e -> e.compareTo(
			settings.minThreshold) >= 0 && e.compareTo(settings.maxThreshold) <= 0)
			.count();
