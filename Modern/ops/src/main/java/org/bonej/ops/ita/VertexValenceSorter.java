...
public class VertexValenceSorter extends
	AbstractBinaryFunctionOp<Graph, Pair<Integer, Integer>, Map<Integer, List<Vertex>>>
	implements Contingent
{

	/**
	 * This function iterates through all vertices in a {@link Graph} and returns
	 * a data structure (a map of lists of vertices} sorting the vertices that
	 * have valence within the specified valence range.
	 * 
	 * @param graph the {@link Graph} from the AnalyzeSkeleton tool whose vertices
	 *          will be mapped by this Op.
	 * @param valenceRange the range of valences that should be kept. The range
	 *          includes both values passed.
	 * @return a map of lists of vertices, the keys of the map specify the valence
	 *         of all the vertices in each list
	 */
	@Override
	public Map<Integer, List<Vertex>> calculate(final Graph graph,
		final Pair<Integer, Integer> valenceRange)
	{
		final Map<Integer, List<Vertex>> valenceMap = new HashMap<>();
		final ArrayList<Vertex> vertices = graph.getVertices();

		vertices.stream().filter(v -> isInValenceRange(v, valenceRange)).forEach(
			v -> {
	...
	private boolean isInValenceRange(final Vertex v,
