...
public class SolveQuadricEqTest {
	...
	public void testSolution() {
		// SETUP
		final double alpha = Math.cos(Math.PI / 4.0);
		final List<Vector3d> unitSpherePoints = Stream.of(new Vector3d(1, 0, 0),
			new Vector3d(-1, 0, 0), new Vector3d(0, 1, 0), new Vector3d(0, -1, 0),
			new Vector3d(0, 0, 1), new Vector3d(0, 0, -1), new Vector3d(alpha, alpha,
				0), new Vector3d(-alpha, alpha, 0), new Vector3d(alpha, -alpha, 0),
			new Vector3d(-alpha, -alpha, 0), new Vector3d(0, alpha, alpha),
		...
		// EXECUTE
		final Matrix4d solution = (Matrix4d) IMAGE_J.op().run(SolveQuadricEq.class,
			unitSpherePoints);

		// VERIFY
		final double a = solution.getElement(0, 0);
		assertEquals("The parameter is incorrect", 1.0, a, 1e-12);
		final double b = solution.getElement(1, 1);
		assertEquals("The parameter is incorrect", 1.0, b, 1e-12);
		final double c = solution.getElement(2, 2);
		assertEquals("The parameter is incorrect", 1.0, c, 1e-12);
		final double d = solution.getElement(0, 1);
		assertEquals("The parameter is incorrect", 0.0, d, 1e-12);
		final double e = solution.getElement(0, 2);
		assertEquals("The parameter is incorrect", 0.0, e, 1e-12);
		final double f = solution.getElement(1, 2);
		assertEquals("The parameter is incorrect", 0.0, f, 1e-12);
		final double g = solution.getElement(0, 3);
		assertEquals("The parameter is incorrect", 0.0, g, 1e-12);
		final double h = solution.getElement(1, 3);
		assertEquals("The parameter is incorrect", 0.0, h, 1e-12);
		final double i = solution.getElement(2, 3);
		assertEquals("The parameter is incorrect", 0.0, i, 1e-12);
