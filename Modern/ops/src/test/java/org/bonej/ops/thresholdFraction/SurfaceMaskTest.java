...
public class SurfaceMaskTest {
	...
	public void testCompute2() throws Exception {
		...
		final Img<LongType> img = IMAGE_J.op().create().img(new FinalDimensions(2,
			2, 2), new LongType());
		final ImgPlus<LongType> imgPlus = new ImgPlus<>(img, "", xAxis, yAxis,
			zAxis);
		...
		final Img<BitType> mask = (Img<BitType>) IMAGE_J.op().run(SurfaceMask.class,
			imgPlus, thresholds);
		final long voxels = StreamSupport.stream(mask.spliterator(), false).filter(
			BitType::get).count();

		assertEquals("Incorrect number of foreground voxels in the mask", 4,
			voxels);
	}
}
