...
public class ElementUtilTest {
    ...
    public void testCalibratedSpatialElementSizeNullSpace() throws Exception {
        final double result = ElementUtil.calibratedSpatialElementSize(null, unitService);
    ...
    public void testCalibratedSpatialElementSizeNoSpatialAxes() throws Exception {
        ...
        final double elementSize = ElementUtil.calibratedSpatialElementSize(imgPlus, unitService);

        assertTrue("Element size should be NaN when there are no spatial axes", Double.isNaN(elementSize));
    }
    ...
    public void testCalibratedSpatialElementSizeNonLinearAxis() throws Exception {
        ...
        final double result = ElementUtil.calibratedSpatialElementSize(imgPlus, unitService);

        assertTrue("Size should be NaN when space has nonlinear axes", Double.isNaN(result));
    }

    @Test
    public void testCalibratedSpatialElementSizeUnitsUnconvertible() throws Exception {
    ...
    public void testCalibratedSpatialElementSizeNoUnits() throws Exception {
        ...
        final ImgPlus<ByteType> imgPlus = new ImgPlus<>(img, "", xAxis, yAxis, zAxis);
    ...
    public void testCalibratedSpatialElementSize() throws Exception {
        final double[][] scales = {{20.0, 4.0, 1.0}, {20.0, 1.0, 4.0}, {4.0, 20.0, 1.0}};
        final String[][] units = {{"mm", "cm", "m"}, {"m", "cm", "mm"}, {"µm", "µm", "µm"}};
        ...
        for (int i = 0; i < scales.length; i++) {
            ...
            assertEquals("Element size is incorrect", expected[i], elementSize, 1e-12);
        }
    }
}