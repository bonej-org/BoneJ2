...
public class ResultsInserterTest {
	...
	@Test
	public void testSetMeasurementInFirstFreeRowAddsARowForNewLabel()
		throws Exception
	{
		final int COLUMN = 0;

		beforeCount = resultsTable.getCounter();
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, MEASUREMENT_HEADING,
			MEASUREMENT_VALUE);
		afterCount = resultsTable.getCounter();

		assertEquals(0, beforeCount);
		assertEquals("Results table should have one more row after insertion", 1,
			afterCount);
		...
		assertEquals("The new column has the wrong heading", MEASUREMENT_HEADING,
			measurementHeading);

		double measurementValue = resultsTable.getValueAsDouble(COLUMN, lastRow);
		assertEquals("The new column has the wrong value", MEASUREMENT_VALUE,
			measurementValue, DELTA);
	}

	@Test
	public void testSetMeasurementInFirstFreeRowAddsARowForRepeatMeasurement()
		throws Exception
	{
		beforeCount = resultsTable.getCounter();
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, MEASUREMENT_HEADING,
		...
		assertEquals(0, beforeCount);
		assertEquals(
			"ResultsInserter should add a new row for a repeat measurement with the same label",
			2, afterCount);
	}

	@Test
	public void testSetMeasurementInFirstFreeRowAddsAColumnForNewMeasurement()
		throws Exception
	{
		beforeCount = resultsTable.getCounter();
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, MEASUREMENT_HEADING,
			MEASUREMENT_VALUE);
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, NEW_MEASUREMENT_HEADING,
			NEW_MEASUREMENT_VALUE);
		afterCount = resultsTable.getCounter();

		assertEquals(
			"Adding multiple measures for the same label should only create one row",
			beforeCount + 1, afterCount);
	}

	@Test
	public void
		testSetMeasurementInFirstFreeRowAddsColumnToTheFirstRowWithTheSameLabel()
			throws Exception
	{
		resultsInserter.setMeasurementInFirstFreeRow("Another label",
			MEASUREMENT_HEADING, MEASUREMENT_VALUE);
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, MEASUREMENT_HEADING,
			MEASUREMENT_VALUE);
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, MEASUREMENT_HEADING,
			MEASUREMENT_VALUE);
		...
		assertEquals("The new measurement was inserted on the wrong row",
			Double.NaN, resultsTable.getValueAsDouble(lastColumn, 2), DELTA);
		assertEquals(
			"The new measurement should have been inserted on the first row with the same label",
			NEW_MEASUREMENT_VALUE, resultsTable.getValueAsDouble(lastColumn, 1),
			DELTA);
	}

	@Test
	public void
		testSetMeasurementInFirstFreeRowAddsColumnValueToTheFirstRowWithNoData()
			throws Exception
	{
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, MEASUREMENT_HEADING,
			MEASUREMENT_VALUE);
		...
		assertEquals(
			"The new value should be inserted on the first row with no data",
			NEW_MEASUREMENT_VALUE, resultsTable.getValueAsDouble(1, 1), DELTA);
		assertEquals("The new value was inserted on the wrong row", Double.NaN,
			resultsTable.getValueAsDouble(1, 2), 0.00000001);
	}

	@Test
	public void testSetMeasurementInFirstFreeRowNaNMeasurementIsMarkedErr()
		throws Exception
	{
		resultsInserter.setMeasurementInFirstFreeRow(LABEL, MEASUREMENT_HEADING,
			Double.NaN);
		...
		assertEquals("NaN value not marked down as " + ResultsInserter.NAN_VALUE,
			ResultsInserter.NAN_VALUE, value);
	}
