...
public class RoiManagerUtilTest {
    ...
    public void testGetSliceRoiReturnEmptyListIfStackNull() {
        final List<Roi> result = RoiManagerUtil.getSliceRoi(MOCK_ROI_MANAGER, null, 1);
    ...
    public void testGetSliceRoiReturnEmptyListIfInvalidNumber() {
        final List<Roi> result = RoiManagerUtil.getSliceRoi(MOCK_ROI_MANAGER, testStack, -3);

        assertTrue("Out of bounds slice number should return no ROIs", result.isEmpty());
    }

    @Test
    public void testGetSliceRoi() throws Exception {
        // Mock a RoiManager with several Rois
        // RoiManager.getSliceNumber tries to parse the number of the slice from the label of the Roi
        ...
        final List<Roi> result = RoiManagerUtil.getSliceRoi(MOCK_ROI_MANAGER, testStack, 4);

        assertEquals("Wrong number of ROIs returned", 3, result.size());
        assertEquals("Wrong ROI returned, or ROIs in wrong order", multiRoi1Label, result.get(0).getName());
        assertEquals("Wrong ROI returned, or ROIs in wrong order", multiRoi2Label, result.get(1).getName());
        assertEquals("Wrong ROI returned, or ROIs in wrong order", noSliceLabel, result.get(2).getName());
    }
    ...
    public void testGetLimitsReturnEmptyIfRoiManagerEmpty() throws Exception {
        ...
        Optional<int[]> result = RoiManagerUtil.getLimits(MOCK_ROI_MANAGER, testStack);
    ...
    public void testCropStackWithPadding() throws Exception {
        ...
        // Create a test image
        final ImageStack stack = createCuboid(10, 10, 10, (byte) 0xFF, 0).getStack();
        final ImageStack unpadded = RoiManagerUtil.cropToRois(MOCK_ROI_MANAGER, stack, false, 0x00, 0).get();

        final ImageStack result = RoiManagerUtil.cropToRois(MOCK_ROI_MANAGER, stack, false, 0x00, PADDING).get();

        assertEquals("Cropped stack has wrong padded width", WIDTH + TOTAL_PADDING, result.getWidth());
        assertEquals("Cropped stack has wrong padded height", HEIGHT + TOTAL_PADDING, result.getHeight());
        assertEquals("Cropped stack has wrong padded depth", DEPTH + TOTAL_PADDING, result.getSize());
        assertTrue("Padding didn't shift the pixels correctly", pixelsShifted(unpadded, result, PADDING));
    }
    ...
    public void testCropStackFillColor() throws Exception {
        ...
        final Optional<ImageStack> optional = RoiManagerUtil.cropToRois(MOCK_ROI_MANAGER, stack, true, FILL_COLOR, 0);
    ...
    public void testCropStack() throws Exception {
        ...
        ImagePlus image = createCuboid(CUBE_SIDE, CUBE_SIDE, CUBE_SIDE, TEST_COLOR, 1);
        ImageStack originalStack = image.getStack();

        //All valid ROIs (basic cropping test)
        Optional<ImageStack> optionalResult =
                RoiManagerUtil.cropToRois(MOCK_ROI_MANAGER, originalStack, false, 0x00, 0);
    ...
    public void testGetSafeRoiBoundsInvalidRoi() throws Exception {
        ...
        final boolean result = RoiManagerUtil.getSafeRoiBounds(rectangle, testStack.getWidth(), testStack.getHeight());
    ...
    public void testGetSafeRoiBounds() throws Exception {
        ...
        final boolean result =
                RoiManagerUtil.getSafeRoiBounds(tooLargeRectangle, testStack.getWidth(), testStack.getHeight());
        ...
        assertEquals("Rectangle width is incorrect", testStack.getWidth() - X, tooLargeRectangle.width);
        assertEquals("Rectangle height is incorrect", testStack.getHeight() - Y, tooLargeRectangle.height);
    }
    ...
    public void testCopyRoiWithMask() throws Exception {
        ...
        // Assert results
        final Optional<ImageStack> optionalResult = RoiManagerUtil.cropToRois(MOCK_ROI_MANAGER, mockStack, false, 0x00);
        assertTrue("Optional should not be empty", optionalResult.isPresent());
        final ImageStack result = optionalResult.get();

        final int foregroundCount = countColorPixels(result, TEST_COLOR);
        assertEquals("Image was cropped incorrectly", TEST_COLOR_COUNT, foregroundCount);
    }
    ...
    private ImagePlus createCuboid(final int width, final int height, final int depth, final byte color,
            final int padding) {
        ...
        final ImagePlus imagePlus = IJ.createImage("Cuboid", "8black", paddedWidth, paddedHeight, paddedDepth);
