...
/**
 * A utility class to create an ImgPlus that follows the conventions of a
 * hyperstack in ImageJ1. Needed so that the image display correctly (at least
 * in the legacy ui).
 *
...
public class IJ1ImgPlus {
	...
	public static ImgPlus<BitType> createIJ1ImgPlus(String title,
		final long width, final long height, final long depth, final long channels,
		final long frames, final long padding, final double scale,
		final String unit)
	{
		final long totalPadding = 2 * padding;
		final Img<BitType> img = ArrayImgs.bits(width + totalPadding, height +
			totalPadding, channels, depth + totalPadding, frames);
