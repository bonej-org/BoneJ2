...
public class IsosurfaceWrapperTest {
    ...
    public void testFileWritingExceptionsShowErrorDialog() throws Exception {
        ...
        when(mockUI.chooseFile(any(File.class), anyString())).thenReturn(exceptionsThrowingFile);
        when(mockUI.dialogPrompt(startsWith(STL_WRITE_ERROR), anyString(), eq(ERROR_MESSAGE), any()))
