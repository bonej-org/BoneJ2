...
public class ViewUtilsTest {
	...
	public void testCreateSpatialViews3D() throws Exception {
		...
		final ImgPlus<ByteType> imgPlus = new ImgPlus<>(img, "Test image", xAxis,
			yAxis, zAxis, cAxis, tAxis);

		final List<SpatialView<ByteType>> views = ViewUtils.createSpatialViews(
			imgPlus);

		assertEquals("Wrong number of spatial views generated", 1, views.size());
		assertTrue("There should be no description of hyper position", views.get(
			0).hyperPosition.isEmpty());
	}
	...
	public void testCreateSpatialViewsChannels() throws Exception {
		...
		assertEquals("Wrong number of spatial views generated", channels, views
			.size());
		...
		for (int c = 0; c < channels; c++) {
			...
			assertEquals("The description of hyper position is incorrect", expected,
				hyperPosition);
		}
	}
	...
	public void testCreateSpatialViewsFrames() throws Exception {
		...
		assertEquals("Wrong number of spatial views generated", frames, views
			.size());
	...
	/**
	 * Test createSpatialViews with an image that has both channel and time
	 * dimensions
	 */
	@Test
	public void testCreateSpatialViews() throws Exception {
		...
		final ImgPlus<ByteType> imgPlus = new ImgPlus<>(img, "Test image", xAxis,
			yAxis, zAxis, tAxis, cAxis);

		final List<SpatialView<ByteType>> views = ViewUtils.createSpatialViews(
			imgPlus);

		assertEquals("Wrong number of spatial views generated", frames * channels,
			views.size());
