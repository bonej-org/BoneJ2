...
public class IntertrabecularAngleWrapperTest {
	...
	public void testNullImageCancelsPlugin() throws Exception {
		CommonWrapperTests.testNullImageCancelsPlugin(IMAGE_J, IntertrabecularAngleWrapper.class);
	}

	@Test
	public void testNonBinaryImageCancelsPlugin() throws Exception {
		CommonWrapperTests.testNonBinaryImagePlusCancelsPlugin(IMAGE_J, IntertrabecularAngleWrapper.class);
	}

	@Test
	public void testCompositeImageCancelsPlugin() throws Exception {
		// SETUP
		final String expectedMessage = CommonMessages.HAS_CHANNEL_DIMENSIONS + ". Please split the channels.";
		...
		final CommandModule module = IMAGE_J.command()
				.run(IntertrabecularAngleWrapper.class, true, "inputImage", imagePlus).get();

		// VERIFY
		assertTrue("A composite image should have cancelled the plugin", module.isCanceled());
		assertEquals("Cancel reason is incorrect", expectedMessage, module.getCancelReason());
	...
	public void testPrintCentroids() throws Exception {
		...
		final ImagePlus line = NewImage.createByteImage("Test", 5, 3, 1, FILL_BLACK);
		...
		final Iterator<String> expectedColumnHeaders = Stream.of("V1x", "V1y", "V1z", "V2x", "V2y", "V2z").iterator();
		centroids.forEach(c -> assertEquals(expectedColumnHeaders.next(), c.getHeader()));
	...
	public void testTimeDimensionCancelsPlugin() throws Exception {
		// SETUP
		final String expectedMessage = CommonMessages.HAS_TIME_DIMENSIONS + ". Please split the hyperstack.";
		final UserInterface mockUI = CommonWrapperTests.mockUIService(IMAGE_J);
		...
		// VERIFY
		assertTrue("An image with time dimension should have cancelled the plugin", module.isCanceled());
		assertEquals("Cancel reason is incorrect", expectedMessage, module.getCancelReason());
		verify(mockUI, after(100)).dialogPrompt(anyString(), anyString(), any(), any());
	}
	...
	public void testNoSkeletonsCancelsPlugin() throws Exception {
		...
		assertEquals("Cancel reason is incorrect", NO_SKELETONS, module.getCancelReason());
		verify(mockUI, after(100)).dialogPrompt(anyString(), anyString(), any(), any());
	}

	/**
	 * Test that no skeleton image pops open, when the input is already a
	...
	public void testNoImageWhenNoSkeletonisation() throws Exception {
		...
		doNothing().when(mockUI).show(any(ImagePlus.class));
		IMAGE_J.ui().setDefaultUI(mockUI);
		final ImagePlus pixel = NewImage.createByteImage("Test", 3, 3, 1, FILL_BLACK);
		pixel.getStack().getProcessor(1).set(1, 1, (byte) 0xFF);

		// EXECUTE
		final CommandModule module = IMAGE_J.command().run(IntertrabecularAngleWrapper.class, true, "inputImage", pixel)
				.get();

		// VERIFY
		assertEquals("Sanity check failed: plugin cancelled before image could have been shown", NO_RESULTS_MSG,
				module.getCancelReason());
		verify(mockUI, after(100).never()).show(any(ImagePlus.class));
	}

	/**
	 * Test that the skeleton is displayed, when the input image gets
	 * skeletonised
	 */
	@Test
	public void testSkeletonImageWhenSkeletonised() throws Exception {
		...
		doNothing().when(mockUI).show(any(ImagePlus.class));
		IMAGE_J.ui().setDefaultUI(mockUI);
		final ImagePlus square = NewImage.createByteImage("Test", 4, 4, 1, FILL_BLACK);
		...
		// EXECUTE
		IMAGE_J.command().run(IntertrabecularAngleWrapper.class, true, "inputImage", square).get();
	...
	public void testNoResultsCancelsPlugin() throws Exception {
		...
		// VERIFY
		assertTrue("Plugin should have cancelled", module.isCanceled());
		assertEquals("Cancel reason is incorrect", NO_RESULTS_MSG, module.getCancelReason());
		verify(mockUI, after(100)).dialogPrompt(anyString(), anyString(), any(), any());
	}

	@Test
	public void testAnisotropicImageShowsWarningDialog() throws Exception {
		CommonWrapperTests.testAnisotropyWarning(IMAGE_J, IntertrabecularAngleWrapper.class);
	}
	...
	public void testMultipleGraphsShowsWarningDialog() throws Exception {
		...
		when(mockUI.dialogPrompt(startsWith("Image has multiple skeletons"), anyString(), eq(WARNING_MESSAGE), any()))
				.thenReturn(mockPrompt);
		IMAGE_J.ui().setDefaultUI(mockUI);
		final ImagePlus pixels = NewImage.createByteImage("Test", 4, 4, 1, FILL_BLACK);
